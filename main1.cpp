...
int main()
{
  ...
  double Rayonvision = 125, Taille = 25, vgroupe = 3; //propriétés, comportement des oiseaux
  
  double Taillesurfacex = 1600., Taillesurfacey = 900.; //taille de l'interface SFML sur laquelle les oiseaux vont se déplacer
  
  int N = 80; //Nombre d'oiseaux dans la nuée
  ...
  for(int i = 0; i < N; i++) { //initialisation de la nuée d'oiseaux
    oiseau O(Rayonvision, Taille, vgroupe, Taillesurfacex, Taillesurfacey, obst);
    nuee.push_back(O);
  }
